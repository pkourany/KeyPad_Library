...
void Keypad::setDebounceTime(unsigned long debounce) {
    debounceTime = debounce;
}

void Keypad::setHoldTime(unsigned long hold) {
    holdTime = hold;
}
...
void Keypad::initializePins() {
    ...
    for (byte R=0; R<size.rows; R++) {
        pinMode(rowPins[R],INPUT_PULLUP);// Enable the internal 20K pullup resistors for each row pin.
    }
}
...
/*
|| @changelog
|| | 2015-03-11 - Paul Kourany : Adapted for Spark Core (www.spark.io)
